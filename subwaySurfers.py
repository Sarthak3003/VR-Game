...
with mp_pose.Pose(
  ...
  while cap.isOpened():
    ...
    frame, results = detectPose(image, pose_video, draw=True)
    if results.pose_landmarks:
        ...
        if game_started:
            ...
            frame, horizontal_position = checkHorizontalPosition(frame, results, draw=True)
        ...
        # Otherwise if the game has not started    
            ...
            cv2.putText(frame, 'JOIN BOTH HANDS TO START THE GAME.', (5, image_height - 10), cv2.FONT_HERSHEY_PLAIN,
                        2, (0, 255, 0), 3)
        ...
        if checkHandsJoined(frame, results)[1] == 'Hands Joined':
            ...
            if counter == num_of_frames:
                ...
                if not(game_started):
                    ...
                    left_y = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_SHOULDER].y * image_height)

                    # Retreive the y-coordinate of the right shoulder landmark.
                    right_y = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.LEFT_SHOULDER].y * image_height)
        ...
        if MID_Y:
            ...
            frame, posture = checkVerticalPosition(frame, results, MID_Y)
...
#     if results.pose_landmarks:
#         # Check if the left and right hands are joined.
#         image, _ = checkHandsJoined(image, results, draw=True)
